...
#                fallocate chdir fchdir chroot fchownat fchown openat pipe2 quotactl lseek read write sendfile splice tee readlinkat
...
#array_syscalls=(epoll_pwait statfs fstatfs readv writev ppoll vmsplice timerfd_settime timerfd_gettime getitimer
#		timer_create timer_gettime timer_settime clock_settime clock_gettime clock_getres clock_nanosleep sched_setparam sched_setscheduler sched_getparam sched_rr_get_interval
#		times getrlimit setrlimit getrusage adjtimex mq_open mq_timedsend mq_timedreceive mq_notify
#		semtimedop semop recvmsg name_to_handle_at open_by_handle_at process_vm_readv process_vm_writev)

array_syscalls=(mq_timedsend)
...
create_glibc_log() {
	...
	cat $trinity_log |grep "parameter value" > $log
...
# compare
#############################################################
result_e=$DIR/log_struct/result.error
result_p=$DIR/log_struct/result.pass
result_c=$DIR/log_struct/result.check
compare()
{
	local ret=0
...
do
	...
	log_dir=$DIR/log_struct/$test_syscall
